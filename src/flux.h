...
struct Value {
    std::variant<Vec, Str, List, Closure, NativeFn, std::nullptr_t> data;
    Value() : data(nullptr) {}
    ...
    bool is_vec()  const {
        return std::holds_alternative<Vec>(data);
    }
    bool is_str()  const {
        return std::holds_alternative<Str>(data);
    }
    bool is_list() const {
        return std::holds_alternative<List>(data);
    }
    bool is_closure() const {
        return std::holds_alternative<Closure>(data);
    }
    ...
    const NativeFn& as_native()  const {
...
struct Env {
    ...
    void def(const std::string& n, Value v) {
        vars[n]=std::move(v);
    }
};
...
enum class Tk {
    ...
    Var, Func, If, Else, While, For, Return, Break, Continue, Print, Load
};
...
[[noreturn]] inline void err(const std::string& f, int ln, const std::string& msg) {
    throw std::runtime_error(f + ":" + std::to_string(ln) + ": " + msg);
}

// ── Path resolution ──────────────────────────────────────────────────
// Resolve a path relative to the directory of the calling script.
...
#endif
    return p; // return the normalized path anyway; caller will report error
}
    ...
    Token next() {
        ...
        if (std::isalpha((unsigned char)c)||c=='_') {
            ...
            static const std::unordered_map<std::string,Tk> kw= {
                ...
                {"while",Tk::While},{"for",Tk::For},{"return",Tk::Return},
        ...
        case '>':
            ...
            return{Tk::Gt,">",ln};
        default:
            err(file,ln,std::string("unexpected char '")+c+"'");
        }
    }
...
enum class NodeT {
    ...
    VarDecl, Assign, FuncDecl, IfStmt, WhileStmt, ForStmt,
...
struct Parser {
    ...
    size_t pos=0;
    std::shared_ptr<std::string> file;
    Parser(std::vector<Token> t, const std::string& f) : tokens(std::move(t)),
        file(std::make_shared<std::string>(f)) {}
    Token& cur() {
        return tokens[pos];
    }
    Token eat(Tk t) {
        if(cur().type!=t) err(*file,cur().line,
    ...
    ExprPtr parse_stmt() {
        ...
        if(check(Tk::Break)) {
            ++pos;
            return make(NodeT::BreakStmt,ln);
        }
        if(check(Tk::Continue)) {
            ++pos;
            return make(NodeT::ContStmt,ln);
        }
    ...
    ExprPtr parse_print() {
        ...
        while(!check(Tk::Eof)&&!check(Tk::RBrace)&&cur().line==ln)e->args.push_back(parse_expr());
        return e;
    }
    ExprPtr parse_assign() {
        auto e=parse_expr();
        if(check(Tk::Eq)&&e->type==NodeT::Id) {
            int ln=cur().line;
            eat(Tk::Eq);
    ...
    static bool callable_node(const ExprPtr& e) {
        return e->type==NodeT::Id||e->type==NodeT::Call||e->type==NodeT::Index||e->type==NodeT::FuncDecl;
    }
...
struct Interpreter {
    ...
    Interpreter() : global(std::make_shared<Env>()) {
    ...
    static void ck(const char* nm,const std::vector<Value>& a,size_t n,int ln,const std::string& f) {
        if(a.size()!=n) err(f,ln,std::string(nm)+" expects "+std::to_string(n)+" arg(s), got "+std::to_string(a.size()));
    }
    ...
    static void nf(const char* nm,const Value& v,int ln,const std::string& f) {
        if(!v.is_closure()&&!v.is_native()) err(f,ln,std::string(nm)+" expects function");
    }
    ...
    Value call_value(const Value& fn, const std::vector<Value>& args, int ln, const std::string& f) {
        ...
        if (fn.is_closure()) {
            ...
            if(args.size()!=cl.params.size()) err(f,ln,(cl.name.empty()?"<anonymous>":cl.name)
                                                      +" expects "+std::to_string(cl.params.size())+" arg(s), got "+std::to_string(args.size()));
            ...
            try {
                ...
                throw std::runtime_error(std::string(re.what())
                                         +"\n  in "+(cl.name.empty()?"<anonymous>":cl.name)+"(), called from "+f+":"+std::to_string(ln));
            }
    ...
    void register_builtins() {
        ...
        reg("len",[](auto& a,int ln,auto& f)->Value{ ck("len",a,1,ln,f);
        ...
        reg("reverse",[](auto& a,int ln,auto& f)->Value{ ck("reverse",a,1,ln,f);
        if(a[0].is_str()) {
        auto s=a[0].as_str();
        ...
        reg("slice",[](auto& a,int ln,auto& f)->Value{ck("slice",a,3,ln,f);
        ...
        reg("concat",[](auto& a,int ln,auto& f)->Value{ck("concat",a,2,ln,f);
                if(a[0].is_str()&&a[1].is_str()) return Value(Str(a[0].as_str()+a[1].as_str()));
        ...
        auto m1=[this](const char* nm, Vec(*op)(const Vec&)) {
            reg(nm,[nm,op](auto& a,int ln,auto& f)->Value{ck(nm,a,1,ln,f); nv(nm,a[0],ln,f); return Value(op(a[0].as_vec()));});
        };
        ...
        reg("floor",[](auto& a,int ln,auto& f)->Value{ck("floor",a,1,ln,f); nv("floor",a[0],ln,f);
                auto& v=a[0].as_vec(); Vec r(v.size()); for(size_t i=0; i<v.size(); ++i)r[i]=std::floor(v[i]); return Value(r);});
        reg("ceil",[](auto& a,int ln,auto& f)->Value{ck("ceil",a,1,ln,f); nv("ceil",a[0],ln,f);
                auto& v=a[0].as_vec(); Vec r(v.size()); for(size_t i=0; i<v.size(); ++i)r[i]=std::ceil(v[i]); return Value(r);});
        reg("round",[](auto& a,int ln,auto& f)->Value{ck("round",a,1,ln,f); nv("round",a[0],ln,f);
                auto& v=a[0].as_vec(); Vec r(v.size()); for(size_t i=0; i<v.size(); ++i)r[i]=std::round(v[i]); return Value(r);});
        reg("pow",[](auto& a,int ln,auto& f)->Value{ck("pow",a,2,ln,f); nv("pow",a[0],ln,f); nv("pow",a[1],ln,f);
                return Value(std::pow(a[0].as_vec(),a[1].as_vec()));});
        reg("sort",[](auto& a,int ln,auto& f)->Value{ck("sort",a,1,ln,f); nv("sort",a[0],ln,f);
                auto v=a[0].as_vec(); std::sort(std::begin(v),std::end(v)); return Value(v);});

        // ── vec: constructors ─────────────────────────────────────────
        reg("range",[](auto& a,int ln,auto& f)->Value{
            if(a.size()<1||a.size()>3)err(f,ln,"range expects 1-3 args");
            nv("range",a[0],ln,f); double start=0,stop,step=1;
            if(a.size()==1) {
                stop=a[0].scalar();
            } else {
                start=a[0].scalar();
            ...
            int n=std::max(0,(int)std::ceil((stop-start)/step)); Vec r(n);
            for(int i=0; i<n; ++i)r[i]=start+i*step; return Value(r);});
        reg("zeros",[](auto& a,int ln,auto& f)->Value{ck("zeros",a,1,ln,f); nv("zeros",a[0],ln,f);
                return Value(Vec(0.0,(size_t)a[0].scalar()));});
        reg("ones",[](auto& a,int ln,auto& f)->Value{ck("ones",a,1,ln,f); nv("ones",a[0],ln,f);
        ...
        reg("rand",[](auto& a,int ln,auto& f)->Value{
            ...
            Vec r(n); for(int i=0; i<n; ++i)r[i]=(double)std::rand()/(double)RAND_MAX; return Value(r);});

        // ── list operations ───────────────────────────────────────────
        reg("list",[](auto& a,int,auto&)->Value{return Value(List(a.begin(),a.end()));});
        reg("push",[](auto& a,int ln,auto& f)->Value{ck("push",a,2,ln,f);
                if(!a[0].is_list())err(f,ln,"push expects list"); List l=a[0].as_list(); l.push_back(a[1]); return Value(l);});

        // ── higher-order: map, filter, reduce, each ───────────────────
        reg("map",[this](auto& a,int ln,auto& f)->Value{ck("map",a,2,ln,f);
                if(!a[0].is_list())err(f,ln,"map expects list"); nf("map",a[1],ln,f);
                List out; for(auto& x:a[0].as_list())out.push_back(call_value(a[1],{x},ln,f)); return Value(out);});
        reg("filter",[this](auto& a,int ln,auto& f)->Value{ck("filter",a,2,ln,f);
                if(!a[0].is_list())err(f,ln,"filter expects list"); nf("filter",a[1],ln,f);
                List out; for(auto& x:a[0].as_list())if(call_value(a[1],{x},ln,f).truthy())out.push_back(x); return Value(out);});
        reg("reduce",[this](auto& a,int ln,auto& f)->Value{ck("reduce",a,3,ln,f);
                if(!a[0].is_list())err(f,ln,"reduce expects list"); nf("reduce",a[1],ln,f);
                Value acc=a[2]; for(auto& x:a[0].as_list())acc=call_value(a[1],{acc,x},ln,f); return acc;});
        reg("each",[this](auto& a,int ln,auto& f)->Value{ck("each",a,2,ln,f);
                if(!a[0].is_list())err(f,ln,"each expects list"); nf("each",a[1],ln,f);
                for(auto& x:a[0].as_list())call_value(a[1],{x},ln,f); return Value(nullptr);});
        reg("apply",[this](auto& a,int ln,auto& f)->Value{ck("apply",a,2,ln,f);
                nf("apply",a[0],ln,f); if(!a[1].is_list())err(f,ln,"apply expects list of arguments");
                std::vector<Value> args(a[1].as_list().begin(),a[1].as_list().end());
                return call_value(a[0],args,ln,f);});

        // ── string operations ─────────────────────────────────────────
        reg("upper",[](auto& a,int ln,auto& f)->Value{ck("upper",a,1,ln,f); ns("upper",a[0],ln,f);
                auto s=a[0].as_str(); for(auto& c:s)c=std::toupper((unsigned char)c); return Value(Str(s));});
        reg("lower",[](auto& a,int ln,auto& f)->Value{ck("lower",a,1,ln,f); ns("lower",a[0],ln,f);
                auto s=a[0].as_str(); for(auto& c:s)c=std::tolower((unsigned char)c); return Value(Str(s));});
        reg("trim",[](auto& a,int ln,auto& f)->Value{ck("trim",a,1,ln,f); ns("trim",a[0],ln,f);
                auto s=a[0].as_str(); auto l=s.find_first_not_of(" \t\n\r");
                if(l==std::string::npos)return Value(Str(""));
                return Value(Str(s.substr(l,s.find_last_not_of(" \t\n\r")-l+1)));});
        reg("split",[](auto& a,int ln,auto& f)->Value{ck("split",a,2,ln,f); ns("split",a[0],ln,f); ns("split",a[1],ln,f);
                List parts; auto& s=a[0].as_str(); auto& d=a[1].as_str(); size_t st=0,p;
        while((p=s.find(d,st))!=std::string::npos) {
        parts.push_back(Value(Str(s.substr(st,p-st))));
            st=p+d.size();
        }
        parts.push_back(Value(Str(s.substr(st)))); return Value(parts);});
        reg("join",[](auto& a,int ln,auto& f)->Value{ck("join",a,2,ln,f);
                if(!a[0].is_list())err(f,ln,"join expects list"); ns("join",a[1],ln,f);
                std::string r; auto& l=a[0].as_list(); auto& sep=a[1].as_str();
        for(size_t i=0; i<l.size(); ++i) {
            if(i)r+=sep;
                r+=l[i].repr();
            }
        return Value(Str(r));});
        reg("substr",[](auto& a,int ln,auto& f)->Value{ck("substr",a,3,ln,f); ns("substr",a[0],ln,f);
                nv("substr",a[1],ln,f); nv("substr",a[2],ln,f);
                return Value(Str(a[0].as_str().substr((size_t)a[1].scalar(),(size_t)a[2].scalar())));});
        reg("find",[](auto& a,int ln,auto& f)->Value{ck("find",a,2,ln,f); ns("find",a[0],ln,f); ns("find",a[1],ln,f);
                auto p=a[0].as_str().find(a[1].as_str()); return Value(p==std::string::npos?-1.0:(double)p);});
        reg("replace",[](auto& a,int ln,auto& f)->Value{ck("replace",a,3,ln,f);
                ns("replace",a[0],ln,f); ns("replace",a[1],ln,f); ns("replace",a[2],ln,f);
                auto s=a[0].as_str(); auto& from=a[1].as_str(); auto& to=a[2].as_str();
        ...
        // ── regex ─────────────────────────────────────────────────────
        reg("match",[](auto& a,int ln,auto& f)->Value{ck("match",a,2,ln,f); ns("match",a[0],ln,f); ns("match",a[1],ln,f);
        try {
        ...
        // ── type / conversion ─────────────────────────────────────────
        reg("type",[](auto& a,int ln,auto& f)->Value{ck("type",a,1,ln,f);
                if(a[0].is_nil())return Value(Str("nil"));
                if(a[0].is_vec()&&a[0].as_vec().size()==1)return Value(Str("scalar"));
                    if(a[0].is_vec())return Value(Str("vec")); if(a[0].is_str())return Value(Str("string"));
                            if(a[0].is_list())return Value(Str("list")); return Value(Str("func"));});
        reg("str",[](auto& a,int ln,auto& f)->Value{ck("str",a,1,ln,f); return Value(Str(a[0].repr()));});
        reg("num",[](auto& a,int ln,auto& f)->Value{ck("num",a,1,ln,f); ns("num",a[0],ln,f);
        try {
            return Value(std::stod(a[0].as_str()));
        } catch(...) {
            err(f,ln,"num: cannot parse '"+a[0].as_str()+"'");
        }});
        reg("vec",[](auto& a,int ln,auto& f)->Value{ck("vec",a,1,ln,f);
        if(a[0].is_list()) {
        auto& l=a[0].as_list();
            Vec v(l.size());
            for(size_t i=0; i<l.size(); ++i) {
        ...
        // ── I/O (all paths normalized relative to calling script) ─────
        reg("read",[](auto& a,int ln,auto& f)->Value{ck("read",a,1,ln,f); ns("read",a[0],ln,f);
                auto p=normalize_path(a[0].as_str(),f);
                std::ifstream fs(p); if(!fs)err(f,ln,"cannot open "+p.string());
                std::ostringstream ss; ss<<fs.rdbuf(); return Value(Str(ss.str()));});
        reg("write",[](auto& a,int ln,auto& f)->Value{ck("write",a,2,ln,f); ns("write",a[0],ln,f);
                auto p=normalize_path(a[0].as_str(),f);
                std::ofstream fs(p); if(!fs)err(f,ln,"cannot open "+p.string());
                fs<<a[1].repr(); return Value(nullptr);});
        reg("append",[](auto& a,int ln,auto& f)->Value{ck("append",a,2,ln,f); ns("append",a[0],ln,f);
                auto p=normalize_path(a[0].as_str(),f);
                std::ofstream fs(p,std::ios::app); if(!fs)err(f,ln,"cannot open "+p.string());
                fs<<a[1].repr(); return Value(nullptr);});

        // ── system ────────────────────────────────────────────────────
        reg("exec",[](auto& a,int ln,auto& f)->Value{ck("exec",a,1,ln,f); ns("exec",a[0],ln,f);
                std::array<char,256> buf; std::string out;
                FILE* p=popen(a[0].as_str().c_str(),"r"); if(!p)err(f,ln,"exec failed");
                while(fgets(buf.data(),buf.size(),p))out+=buf.data(); pclose(p); return Value(Str(out));});
        reg("exit",[](auto& a,int ln,auto& f)->Value{
            int code=0; if(!a.empty()) {
                nv("exit",a[0],ln,f);
                code=(int)a[0].scalar();
            }
            std::exit(code); return Value(nullptr);});
        reg("env",[](auto& a,int ln,auto& f)->Value{ck("env",a,1,ln,f); ns("env",a[0],ln,f);
                auto* v=std::getenv(a[0].as_str().c_str()); return v?Value(Str(v)):Value(nullptr);});
        reg("clock",[](auto& a,int,auto&)->Value{(void)a;
                auto t=std::chrono::high_resolution_clock::now();
                return Value((double)std::chrono::duration_cast<std::chrono::microseconds>(t.time_since_epoch()).count()/1e6);});
        reg("error",[](auto& a,int ln,auto& f)->Value{ck("error",a,1,ln,f); err(f,ln,a[0].repr());});
        reg("assert",[](auto& a,int ln,auto& f)->Value{
            if(a.empty())err(f,ln,"assert expects at least 1 arg");
            if(!a[0].truthy())err(f,ln,"assertion failed"+(a.size()>1?": "+a[1].repr():""));
            return Value(nullptr);});
    ...
    Value vec_binop(const Value& a,const Value& b,const std::string& op,int ln,const std::string& f) {
        ...
        if(vb.size()==1&&va.size()>1) {
            Vec t(va.size());
            t=vb[0];
            vb=t;
        }
        if(va.size()!=vb.size())err(f,ln,"vector size mismatch ("+std::to_string(va.size())+" vs "+std::to_string(vb.size())+")");
        Vec r(va.size());
        if     (op=="+")r=va+vb;
        else if(op=="-")r=va-vb;
        else if(op=="*")r=va*vb;
            ...
            for(size_t i=0; i<va.size(); ++i)r[i]=std::fmod(va[i],vb[i]);
        } else if(op=="==") {
            for(size_t i=0; i<va.size(); ++i)r[i]=va[i]==vb[i];
        } else if(op=="!=") {
            for(size_t i=0; i<va.size(); ++i)r[i]=va[i]!=vb[i];
        } else if(op=="<") {
            for(size_t i=0; i<va.size(); ++i)r[i]=va[i]<vb[i];
    ...
    Value eval(ExprPtr e, EnvPtr env) {
        ...
        if(stack_depth>max_stack) err(e->src_file(),e->line,"stack overflow (depth "+std::to_string(max_stack)+")");
        auto& f=e->src_file();
        int ln=e->line;
        switch(e->type) {
        case NodeT::Num:
            return Value(e->num_val);
        case NodeT::Str:
            return Value(e->str_val);
        case NodeT::Id: {
        ...
        case NodeT::IfStmt:
            if(eval(e->left,env).truthy())exec_block(e->body,std::make_shared<Env>(env));
            else if(!e->args.empty())eval(e->args[0],env);
            else if(e->right)exec_block(e->right->body,std::make_shared<Env>(env));
            return Value(nullptr);
        case NodeT::WhileStmt:
            while(eval(e->left,env).truthy()) {
                try {
                    exec_block(e->body,std::make_shared<Env>(env));
                } catch(BreakSignal&) {
                    break;
        ...
        case NodeT::ForStmt: {
            ...
            eval(e->args[0],scope);
            while(eval(e->args[1],scope).truthy()) {
                try {
                    exec_block(e->body,std::make_shared<Env>(scope));
                } catch(BreakSignal&) {
                    break;
                } catch(ContinueSignal&) {} eval(e->args[2],scope);
            }
            return Value(nullptr);
        }
    ...
    void exec_block(const std::vector<Stmt>& stmts,EnvPtr env) {
        for(auto& s:stmts)eval(s.expr,env);
    }
